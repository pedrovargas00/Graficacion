...
void Dementor::draw(){
    ...
    glPushMatrix();
     //  printf("\ncoord Dementor x: %f\n coord Dementor z: %f\n", CoordDX, CoordDZ);
        ...
        glColor3f(0, 0, 0.5);
        dem = objeto.loadObj("C:/Users/Pedro/Desktop/Graficación/ProyectoGraf/Dementor.obj");
        glCallList(dem);
        //glutSolidCube(1);
    glPopMatrix();
    glColor3f(1, 1, 1);
...
void Dementor::drawDragon(){
    ...
    glPushMatrix();
     //  printf("\ncoord Dementor x: %f\n coord Dementor z: %f\n", CoordDX, CoordDZ);
        ...
        glColor3f(0, 0, 0.5);
        //glutSolidCube(1);
        dragon.trasladar(CoordDX, ty, CoordDZ);
        dragon.rotar(grados, rx, ry, rz);
        dragon.escalar(sx, sy, sz);
        dragon.dragon();
    glPopMatrix();
    glColor3f(1, 1, 1);
}
...
//izquierda, arriba, derecha, abajo
    opc1[0] = CoordDX - 0.1 - retroceso;
    opc1[1] = CoordDZ;
  //  printf("\nopcion 1: %f \t %f", opcion[0][0], opcion[0][1]);
    opc2[0] = CoordDX;
    opc2[1] = CoordDZ - 0.1 - retroceso;
    //printf("\nopcion 2: %f \t %f", opcion[1][0], opcion[1][1]);
    opc3[0] = CoordDX + 0.1 - retroceso;
    opc3[1] = CoordDZ;
    //printf("\nopcion 3: %f \t %f", opcion[2][0], opcion[2][1]);
    opc4[0] = CoordDX;
    opc4[1] = CoordDZ + 0.1 - retroceso;
...
void Dementor::retroceder(Bonos *bono){

    for(int i = 0; i < 3; i++){
        if(bono[i].getTocado()){
            retroceso += 0.2;
            bono[i].setTocado(false);
        }
    }
}
